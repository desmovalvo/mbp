...
def virtualiser(kp_port, sib_port):

    host = "localhost"
    kp_addr = (host, kp_port)
    sib_addr = (host, sib_port)
    
    print "kp addr: " + str(kp_addr)
    print "sib addr: " + str(sib_addr)
    ...
    sib_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sib_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    sib_socket.setsockopt(socket.SOL_SOCKET, socket.SO_KEEPALIVE, 1)
    # sib_socket.bind(sib_addr)
    # sib_socket.listen(2)
    logger.info('Server waiting for publishers on port ' + str(sib_port))
        
    # avvio 2 thread: uno che gestisce i messaggi provenienti dalle
    # virtual sib componenti, e l'altro per i messaggi dei kp
    thread.start_new_thread(kp_handler, (kp_socket))
    thread.start_new_thread(sibs_handler, (sib_socket))


def kp_handler(kp_socket):
