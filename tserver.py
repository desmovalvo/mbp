...
def handler(clientsock, addr):
    ...
    while 1:
        try:
            ...
            if len(ssap_msg) == 1:
                if ssap_msg == " ":
                    if clientsock in sib_list:
                        print colored("tserver> ", "blue", attrs=["bold"]) + str(clientsock) + " is alive "                    
                        sib_list_timers[str(clientsock)] = datetime.datetime.now()
            ...
            # check whether we received a blank message
                ...
                try:
                    ...
                    if info["message_type"] == "REQUEST" and info["transaction_type"] == "REGISTER":
                        if handle_register_request(logger, clientsock, info, sib_list, sib_list_timers):
...
# def observer():
#     try:
#         threading.Timer(5, observer).start()
#         print colored("observer> ", "blue", attrs=["bold"]) + "Checking how many SIBs are alive"

#         for publisher in sib_list:            
#             publisher.send(" ")
#             message = publisher.recv(1024)
